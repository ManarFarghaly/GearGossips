"""
PHASE 2b — Mel-Spectrogram + Statistical Features  (global normalization)   phase 2b only 
══════════════════════════════════════════════════════════════════════════════
All v2 improvements from Phase 2b
...
[FIX 3] Effective Number of Samples (ENS) class weights.
[FIX 4] Label smoothing ε = 0.1.
[FIX 5] Focal loss (γ = 2.0) on the fault-status auxiliary head.
[FIX 6] Hierarchical auxiliary loss (machine-ID head). Weights: 0.4*L_machine + 0.6*L_fault + L_main
[FIX 7] Mixup α = 0.1.
[FIX 8] ReduceLROnPlateau replaces cosine annealing.
[FIX 9] Early stopping on val_loss (patience = 5).
[KEPT]  SpecAugment (freq_mask=27, n_freq=2, time_mask=15, n_time=2)
[KEPT]  Gradient clipping max_norm=1.0
[KEPT]  Weight decay 5e-4, Warmup 2 epochs, Differential LRs
"""
...
PHASE1_CKPT = "/kaggle/input/datasets/hanyfargally/phase1-best-pth/phase1_best.pth"
...
# ── GLOBAL STAT SCALER ────────────────────────────────────────────────────────
# One (mean, std) fitted on all training samples, applied to every sample at inference.
# Use this when machine-to-machine baseline shifts are small or when deployment
# simplicity is preferred over per-machine accuracy.
...
class PrecomputedDatasetMS(Dataset):
    """
    Returns (mel, stat_raw, label, machine_id, fault_label) per sample.
    Global normalization is applied in the training loop — stat is returned raw.
    """
...
class MelStatCNN(nn.Module):
    ...
    Global normalization: the caller normalises stat before passing it in.
    """
...
def load_phase1_weights(model, ckpt_path, device):
    ...
    Phase 1 v3 keys (top-level): block*.*, fc1.*, dropout.*, head_*.*, ...
    Phase 2b mel_stream keys: mel_stream.block*.*, mel_stream.fc1.*, ...
    Prefix 'mel_stream.' to every Phase 1 key, load with strict=False.
    """
    ...
    if new_keys:
        print(f"[ckpt] New layers (randomly init): {new_keys[:6]}{'...' if len(new_keys)>6 else ''}")
...
# Step G: model + load Phase 1 v3 weights
