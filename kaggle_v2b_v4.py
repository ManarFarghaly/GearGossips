...
if USE_WANDB:
    import wandb
    wandb.login()   # set WANDB_API_KEY env var before running
else:
    class _WandbStub:
...
# -- Regularisation -----------------------------------------------------------
LABEL_SMOOTH = 0.1; ENS_BETA = 0.99; MIXUP_ALPHA = 0.4
FOCAL_GAMMA  = 1.0        # fault head (BinaryFocalLoss) only
HIER_ALPHA   = 0.3        # weight of machine-ID auxiliary loss
WEIGHT_DECAY = 1e-3

# -- Learning Rates -----------------------------------------------------------
LR_MEL_STREAM = 1e-6      # very low -- gentle backbone fine-tuning after unfreeze
LR_NEW_LAYERS = 5e-5      # stat_branch, fusion fc1, and all heads

# -- Schedulers / Early Stopping ----------------------------------------------
WARMUP_EPOCHS = 0; RLROP_PATIENCE = 6; RLROP_FACTOR = 0.5
ES_PATIENCE   = 15; UNFREEZE_EPOCH = 2    # backbone unfrozen FROM this epoch
...
sweep_config = {
    'method': 'bayes', # Bayesian optimization (smart guessing based on past runs)
    'metric': {
        'name': 'val_acc',
        'goal': 'maximize'
    },
    'parameters': {
        # The parameters W&B will tweak
        'lr_new_layers': {'values': [5e-5, 1e-4, 3e-4]},
...
# 2. Initialize the sweep on the W&B servers
sweep_id = wandb.sweep(sweep_config, project="machine-fault-phase2b-v4")
print(f"Sweep ID initialized: {sweep_id}")

def train_sweep():
    # Initialize a new W&B run for this specific combination
    with wandb.init():
        # W&B automatically injects the current hyperparams into wandb.config
        config = wandb.config
        
        # 1. Initialize a fresh model for this run
        model = MelStatCNN(num_classes=6, stat_dim=STAT_DIM).to(DEVICE)
        load_phase1_weights(model, PHASE1_CKPT, DEVICE)
        ...
        # 2. Setup Optimizer using the W&B config learning rate
        optimizer = torch.optim.AdamW([
            {"params": model.mel_stream.parameters(),  "lr": 1e-6}, # Backbone LR stays tiny
            {"params": model.stat_branch.parameters(), "lr": config.lr_new_layers},
            {"params": model.fc1.parameters(),         "lr": config.lr_new_layers},
            {"params": model.head_main.parameters(),   "lr": config.lr_new_layers},
            {"params": model.head_machine.parameters(),"lr": config.lr_new_layers},
            {"params": model.head_fault.parameters(),  "lr": config.lr_new_layers},
        ], weight_decay=WEIGHT_DECAY)

        # 3. Setup Loss Functions 
        # ENS_BETA is hardcoded to 0.99 to prevent the abnormal bias collapse
        class_weights = effective_num_weights(label_counts, beta=0.99).to(DEVICE)
        crit_main = nn.CrossEntropyLoss(weight=class_weights, label_smoothing=config.label_smooth)
        ...
        for epoch in range(1, TRAIN_EPOCHS + 1):
            ...
            # Golden Unfreeze Timing
            if epoch == 3:
                for param in model.mel_stream.parameters(): 
                    param.requires_grad = True
                    
            # Train and Eval using the W&B config parameters
            tr_loss, tr_acc = train_epoch(
                model, tr_ldr, optimizer, crit_main, crit_machine, crit_fault, 
                scalers, DEVICE, mixup_alpha=config.mixup_alpha, hier_alpha=config.hier_alpha
            )
            ...
            if es_counter >= 15: # ES_PATIENCE
                break
            
# Launch the agent. 'count' is the maximum number of combinations it will test.
# W&B will automatically stop if it tests 15 combinations.
print("🚀 Starting W&B Sweep Agent...")
wandb.agent(sweep_id, function=train_sweep, count=15)

if USE_WANDB:
    wandb.init(project="machine-fault-phase2b-v4",
               config={"version": "v4_attn_mfcc", "batch_size": BATCH_SIZE,
...
# ==========================================
# 1. Log Final Metrics & W&B Native Confusion Matrix
# ==========================================
if USE_WANDB:
    wandb.log({
...
# ==========================================
# 2. Upload Matplotlib Curves to W&B
# ==========================================
if USE_WANDB:
    wandb.log({"training_curves_plot": wandb.Image(fig)})

plt.show()

# ==========================================
# 3. Safely Close the W&B Run HERE
# ==========================================
if USE_WANDB:
    wandb.finish()
    
    
scaler_path = os.path.join(MODELS_DIR, "stat_scaler_v4.pkl")
...
print("Key files: phase2b_v4_best.pth, stat_scaler_v4.pkl")
